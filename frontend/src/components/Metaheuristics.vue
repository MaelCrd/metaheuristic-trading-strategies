<template>
    <v-col class="main-content">
        <v-row align="center" justify="space-between" class="mt-2 mr-8">
            ...
            <div>
                <v-btn variant="plain" :prepend-icon="showHidden ? 'mdi-eye-off-outline' : 'mdi-eye-outline'"
                    color="primary" @click="handleShowHidden">
        ...
        <v-row>
            <v-data-table multi-sort :headers="headers" :items="filteredItems" class="pl-6 pr-12 pb-6">
                ...
                <template v-slot:item.mh_parameters="{ item }">
                    <v-chip v-for="(param, index) in JSON.parse(item.mh_parameters)" :key="index" size="small"
...
<script lang="ts">
    ...
    export default {
        ...
        computed: {
            ...
            filteredItems() {
                return this.showHidden ? this.items : this.items.filter((item: any) => !item.hidden);
            },
        },
        mounted() {
        ...
        methods: {
            ...
            hideItem(item: any) {
                ...
                axios.put(`http://localhost:9797/api/mh_object?id=${item.id}&hidden=${!item.hidden}`)
                    .then(() => {
